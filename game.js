...
## 输出结构（每次回复严格按此顺序）
① 位置确认：一句话确认玩家所在场景
② 环境描写：2-3句感官描写，以场景速查为基础，可扩展一两个合理的感官细节（光照变化、细微声响、物品质感），但核心物件和空间布局必须严格对照速查
③ 可察细节：1-2个玩家能交互的具体物件（门、抽屉、物品、人），只说"你可以看到/听到/闻到什么"，不替玩家做决定
④ 行动线索（可选）：环境描写结束后，可以自然地提及一个可能引起玩家注意的细节，给玩家行动方向但不替玩家选择（如"前台上那朵枯萎的干花让你多看了一眼"）。不强制要求每条回复都给出。
总字数：200-350字。不要超过。
...
async function sendMessage() {
  ...
  if (npcResp) { addNarratorMsg(npcResp); messages.push({role:"system", content:"【刚才NPC说了以下内容，请在此基础上继续叙述当前环境，不要复述NPC的话，自然地推进场景】"+npcResp}); }
...
function startGame() {
  ...
  messages = [{
    ...
    role: "user", content:"（游戏开始。玩家在雨夜抵达旅馆、推门而入。玛丽安娜站在前台后面，微笑着给玩家登记，递过3号房的钥匙。她用自然的语气告诉玩家镇上的基本规矩——不要像背说明书，要像叮嘱一个远道而来的客人：晚上10点前要回到旅馆；路上遇到居民打招呼要回礼，他们递的点心要接；镇西边没有门牌号的小巷不要进去；如果街上看到一个和自己长得一模一样的人，别对视，低头走开；一定要住满三天才能从镇东牌坊离开。说完这些，她看着你的眼睛，补了一句——记住了吗？然后简短描写大堂环境收束，不要问玩家要做什么。）"
  }];
